...
class UserManager(BaseUserManager):
    ...
    def create_user(self, email, password, **extra_fields):
        ...
        extra_fields.setdefault('role', get_user_model().STUDENT)
...
class User(AbstractBaseUser):
    ...
    username = models.CharField(max_length=191, blank=False, verbose_name='Имя пользователя', unique=True)
...
class Enrollment(models.Model):
    ...
    _COURSE_ROLE_CHOICES = (
        (MANAGER, 'Менеджер'),
        (CLIENT, 'Клиент'),
    )

    _ROLES_TRANSLATION = dict(_COURSE_ROLE_CHOICES)
    ...
    role = models.IntegerField(choices=_COURSE_ROLE_CHOICES, default=CLIENT)
